...
export async function m3u8Merger({url, dir, isConvert, format, maxDownloads}: {
  dir: string;
  isConvert?: boolean;
  url: string;
  format: string;
  ...
  downloadTs({ tsList, urlPrefix, storageDir, isConvert: isConvert !== false, format, maxDownloads });
}
